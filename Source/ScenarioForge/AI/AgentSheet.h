...
#include "AgentSheet.generated.h"
...
public:
	...
	/** Maximum distance an observed enemy may drift from its anchor while still counting as stationary. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Grenade Properties", meta = (Units = "Centimeters", ClampMin = "0.0", UIMin = "0.0", DisplayName = "Stationary Target Radius"))
	float StationaryTargetRadius = 0.0f;

	/** Time an observed enemy must remain inside StationaryTargetRadius before becoming a stationary grenade target. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Grenade Properties", meta = (Units = "Seconds", ClampMin = "0.0", UIMin = "0.0", DisplayName = "Stationary Target Duration"))
	float StationaryTargetDuration = 0.0f;
...
public:
	...
	/** Maximum angle between the muzzle and desired target direction at which weapon fire is visually allowed. Zero disables the firing gate. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Aiming Properties", meta = (Units = "Degrees", ClampMin = "0.0", ClampMax = "180.0", UIMin = "0.0", UIMax = "45.0", DisplayName = "Allowable Fire Aim Deviation"))
	float AllowableFireAimDeviation = 0.0f;

	/** Maximum time the FireWeapon action waits for muzzle alignment before failing and replanning. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Aiming Properties", meta = (Units = "Seconds", ClampMin = "0.0", UIMin = "0.0", DisplayName = "Fire Alignment Timeout"))
	float FireAlignmentTimeout = 0.0f;
};
...
/**
 * @brief Defines melee reach and damage for an agent.
 */
USTRUCT(BlueprintType)
struct SCENARIOFORGE_API FMeleeProperties
{
	GENERATED_BODY()

public:
	/** Maximum distance at which the current target is considered in melee range. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Melee", meta = (Units = "Centimeters", ClampMin = "0.0", UIMin = "0.0", DisplayName = "Melee Range"))
	float Range = 0.0f;

	/** Damage sheet sampled once for each agent hit during a melee swing. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Melee", meta = (DisplayName = "Melee Damage Effect"))
...
public:
	...
	/** Whether the agent can enter a downed state before reaching zero health. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (InlineEditConditionToggle))
	bool bCanBeDowned = false;

	/** Health value at or below which the agent can enter its downed state. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta = (EditCondition = "bCanBeDowned", ClampMin = "0.0", UIMin = "0.0"))
	float DownedHealthThreshold = 0.0f;
};

/**
 * @brief Stores the actions and per-weapon-sheet properties available to an agent.
 */
UCLASS()
class SCENARIOFORGE_API UAgentSheet : public UDataAsset
{
	GENERATED_BODY()
...
public:
	...
	TObjectPtr<UAgentSheet> Parent;

	/** Whether this sheet overrides its parent's pawn sheet sheet. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Pawn", meta = (InlineEditConditionToggle))
	bool bOverridePawnSheet = false;

	/** Pawn presentation sheet used by this agent. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Pawn", meta = (EditCondition = "bOverridePawnSheet"))
	TObjectPtr<UPawnSheet> PawnSheet;
	...
	TObjectPtr<UWeaponSheet> DefaultPrimaryWeapon;
	...
	TObjectPtr<UWeaponSheet> DefaultSecondaryWeapon;
	...
	/** Whether this sheet overrides its parent's stationary-target debug setting. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Grenade Properties|Debug", meta = (InlineEditConditionToggle))
	bool bOverrideDrawStationaryTargetDebug = false;

	/** Draws stationary anchors, configured radii, accumulated time, and grenade-target qualification. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Grenade Properties|Debug", meta = (DisplayName = "Draw Stationary Target Debug", EditCondition = "bOverrideDrawStationaryTargetDebug"))
	bool bDrawStationaryTargetDebug = false;
	...
	/** Whether this sheet overrides its parent's melee settings. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Melee", meta = (InlineEditConditionToggle))
	bool bOverrideMeleeProperties = false;

	/** Melee reach and damage settings used by this agent. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Melee", meta = (EditCondition = "bOverrideMeleeProperties"))
	FMeleeProperties MeleeProperties;
	...
	/** Properties associated with each weapon sheet sheet this agent can use. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Weapons", meta = (EditCondition = "bOverrideWeaponProperties"))
	TMap<UWeaponSheet*, FWeaponProperties> WeaponProperties;
	...
	/** Gets the stationary-target debug setting after resolving parent-sheet inheritance. */
	bool GetResolvedDrawStationaryTargetDebug() const;
	...
	UWeaponSheet* GetResolvedDefaultPrimaryWeapon() const;

	/** Gets the default secondary weapon after resolving parent-sheet inheritance. */
	UWeaponSheet* GetResolvedDefaultSecondaryWeapon() const;

	/** Gets pawn sheet after resolving parent-sheet inheritance. */
	...
	/** Gets melee settings after resolving parent-sheet inheritance. */
	const FMeleeProperties& GetResolvedMeleeProperties() const;
	...
	const TMap<UWeaponSheet*, FWeaponProperties>& GetResolvedWeaponProperties() const;

	/** Finds weapon properties after resolving parent-sheet inheritance. */
	const FWeaponProperties* FindResolvedWeaponProperties(const UWeaponSheet* WeaponSheet) const;

private:
	/** @brief Resolves the action map while preventing parent inheritance cycles. */
	const TMap<TSubclassOf<UAction>, float>& GetResolvedActions(TSet<const UAgentSheet*>& Visited) const;
	 ...
	 */
	void AppendResolvedGoals(TSet<const UAgentSheet*>& Visited, TMap<TSubclassOf<UGoal>, float>& OutGoals) const;

	/** @brief Resolves goal-change debug output while preventing parent inheritance cycles. */
	bool GetResolvedDrawGoalChangeDebug(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves planner state-change debug output while preventing parent inheritance cycles. */
	bool GetResolvedDrawStateChangeDebug(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves stationary-target debug output while preventing parent inheritance cycles. */
	bool GetResolvedDrawStationaryTargetDebug(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves grenade properties while preventing parent inheritance cycles. */
	const TMap<EGrenadeType, FGrenadeProperties>& GetResolvedGrenadeProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves starting grenades while preventing parent inheritance cycles. */
	const TArray<FStartingGrenade>& GetResolvedStartingGrenades(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves the default primary weapon while preventing parent inheritance cycles. */
	UWeaponSheet* GetResolvedDefaultPrimaryWeapon(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves the default secondary weapon while preventing parent inheritance cycles. */
	UWeaponSheet* GetResolvedDefaultSecondaryWeapon(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves pawn sheet while preventing parent inheritance cycles. */
	const UPawnSheet* GetResolvedPawnSheet(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves appearance settings while preventing parent inheritance cycles. */
	const FAppearance& GetResolvedAppearance(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves faction while preventing parent inheritance cycles. */
	EFaction GetResolvedFaction(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves perception settings while preventing parent inheritance cycles. */
	const FPerception& GetResolvedPerception(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves aiming settings while preventing parent inheritance cycles. */
	const FAimingProperties& GetResolvedAimingProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves melee settings while preventing parent inheritance cycles. */
	const FMeleeProperties& GetResolvedMeleeProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves engagement settings while preventing parent inheritance cycles. */
	const FEngageProperties& GetResolvedEngageProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves cover settings while preventing parent inheritance cycles. */
	const FCoverProperties& GetResolvedCoverProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves dodge settings while preventing parent inheritance cycles. */
	const FDodgeProperties& GetResolvedDodgeProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves fired-upon settings while preventing parent inheritance cycles. */
	const FFiredUponProperties& GetResolvedFiredUponProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves vitality settings while preventing parent inheritance cycles. */
	const FVitalityProperties& GetResolvedVitalityProperties(TSet<const UAgentSheet*>& Visited) const;

	/** @brief Resolves weapon properties while preventing parent inheritance cycles. */
	const TMap<UWeaponSheet*, FWeaponProperties>& GetResolvedWeaponProperties(TSet<const UAgentSheet*>& Visited) const;
};
